...
export default function StudySessionScreen() {
  ...
  const checkForActiveSession = async () => {
    try {
      const savedState = await AsyncStorage.getItem(STUDY_SESSION_KEY);
      const hasSession = !!savedState;
  ...
  const saveState = async () => {
    try {
      ...
      console.log('[Study Session] 💾 Saving state:', { 
        isTimerRunning, 
        isPaused, 
        elapsedSeconds,
      ...
      await AsyncStorage.setItem(STUDY_SESSION_KEY, JSON.stringify(state));
      setHasActiveSession(true);
